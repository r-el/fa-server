...
export class Camera {
   ...
   */
  static async create(cameraData: ICamera) {
   ...
   */
  static async getById(cameraId: string) {
   ...
   */
  static async assignToUser(assignmentData: ICameraAssignment) {
    const { data, error } = await supabase
      .from("camera_user_assignments")
      .insert([
        {
   ...
   */
  static async removeAssignment(cameraId: string, userId: string) {
   ...
   */
  static async update(cameraId: string, updateData: Partial<ICamera>) {
    const { data, error } = await supabase
      .from("cameras")
      .update({
        ...updateData,
